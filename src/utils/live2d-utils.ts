...
export class Live2DModelManager {
   ...
   */
  async downloadModelFromGitHub(modelUrl: string, savePath: string): Promise<boolean> {
    try {
...
export const Live2DUtils = {
  ...
  async getModelsFromGitHub(repoUrl: string): Promise<string[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`无法获取GitHub仓库内容: ${response.statusText}`);
      }

      const contents = await response.json();
      const modelDirs = contents
        .filter((item: any) => item.type === 'dir' && item.name !== '.git')
