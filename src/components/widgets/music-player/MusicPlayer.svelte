<script lang="ts">
	...
	onMount(() => {
		...
		const handleStateRequest = () => {
			musicPlayerStore.broadcastCurrentState();
		};
		window.addEventListener("music-player:request-state", handleStateRequest);

		return () => {
			window.removeEventListener("music-player:request-state", handleStateRequest);
		};
	});
