...
export interface SiteConfig {
	...
	banner: {
		...
		imageApi?: {
			...
			url?: string; // 通用主API地址，返回每行一个图片链接的文本
			fallbackUrl?: string; // 通用备用API地址，主API失败时自动切换
			desktopUrl?: string; // 桌面端主API地址
			mobileUrl?: string; // 移动端主API地址
			fallbackDesktopUrl?: string; // 桌面端备用API地址
			fallbackMobileUrl?: string; // 移动端备用API地址
			timeoutMs?: number; // 单次API请求超时时间
			requestParams?: Record<string, string | number | boolean>; // 主/备API共用请求参数，确保格式与尺寸一致
		};
		homeText?: {
